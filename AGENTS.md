...
| 项目层 | `memory/projects.md` | 项目状态、持仓 | 项目有进展时更新 |
| 基础设施层 | `memory/infra.md` | 服务器、API配置 | 配置变化时更新 |
| 教训层 | `memory/lessons.md` | 踩过的坑 | 踩坑后写入 |
| 日志层 | `memory/YYYY-MM-DD.md` | 每日记录 | 记结论不记过程 |
| 学习层 | `.learnings/` | 错误和学习 | 出错时记录 |

### 写入规则

1. **每个信息只记一个地方** - 不要重复
2. **日志记结论不记过程** - 精简便于检索
3. **想记住就写文件** - "Mental notes" don't survive session restarts
4. **标签便于 memorySearch 检索** - 用 #tag 格式
...
- When someone says "remember this" → 写入对应层级文件
- When you learn a lesson → update `memory/lessons.md`
- When you make a mistake → 记录到 `.learnings/ERRORS.md`
- **Text > Brain** 📝
